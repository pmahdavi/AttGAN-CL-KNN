...
def prepare_data(data):
    imgs,imgs_1,captions, captions_lens, class_ids, keys, captions_2, captions_lens_2 = data #pm
    ...
    for i in range(len(imgs_1)):
        imgs_1[i] = imgs_1[i][sorted_cap_indices]
        if cfg.CUDA:
            real_imgs_n.append(Variable(imgs_1[i]).cuda())
        else:
            real_imgs_n.append(Variable(imgs_1[i]))
    #pm
    ...
    return [real_imgs, real_imgs_n, real_imgs_2, captions, sorted_cap_lens,
...
class TextDataset(data.Dataset):
    def __init__(self, data_dir, split='train',
        ...
        self.nn_key = self.load_nn_key("NN_key.pkl")       ## PM
    ...
    def load_nn_key(self,nn_key_dir): #PM
        file_to_read = open("NN_key.pkl", "rb")
        nn_key = pickle.load(file_to_read)
        return nn_key
    ...
    def __getitem__(self, index):
        ...
        # PM: found the next neighbour
        key_1 = key
        if key_1 in self.nn_key:
            key_1 = self.nn_key[key]
        img_name_1 = '%s/images/%s.jpg' % (data_dir, key_1)
        imgs_1 = get_imgs(img_name_1, self.imsize,
                        bbox, self.transform, normalize=self.norm)
        ...
        return imgs, imgs_1 , caps, cap_len, cls_id, key, caps_two, cap_len_two
